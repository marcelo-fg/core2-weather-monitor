...
def update_time_ui(page):
    if page != 0:
        return
    t = ntp_now()
    clock_str = "{:02d}:{:02d}".format(t.get("h", 0), t.get("m", 0))
    # On redessine uniquement la petite horloge jaune du haut
    lcd.rect(0, 30, 90, 28, COL_PANEL, COL_PANEL)
    lcd.font(FONT_MEDIUM)
...
def run_voice_qa(indoor, weather, history, time_now):
    ...
    Triggers the AI announcement. We send a lightweight context 
    so ujson.dumps doesn't crash or freeze the ESP32 due to lack of RAM.
    """
    ai_context = {
        "temperature": indoor.get("temperature"),
        "humidity": indoor.get("humidity"),
        "aq_label": indoor.get("aq_label"),
        "eco2": indoor.get("eco2"),
        "time": time_now,
        "outdoor_temp": weather.get("temp"),
        "outdoor_desc": weather.get("description")
    }
...
def main():
    ...
    while True:
        ...
        # Refresh clock every second (prevents frozen time)
        if now - last_clock >= CLOCK_TICK:
            update_time_ui(page)
            last_clock = now
            ...
            if cmds:
                for c in cmds:
                    ...
                    if ctype == "set_page":
                        ...
                        lcd.rect(0, 190, 320, 50, COL_BG, COL_BG)
                        lcd.font(FONT_SMALL)
                        lcd.print("Reponse...", 5, 195, COL_CYAN)
                        voice_speak("...", audio_id=cval)
                        lcd.rect(0, 190, 320, 50, COL_BG, COL_BG)
            last_poll = now
        ...
        if btnB.wasPressed():
            if page == 3 and wifi_networks:
                ...
                lcd.rect(0, 190, 320, 50, COL_BG, COL_BG)
                lcd.font(FONT_SMALL)
                lcd.print("Use phone to talk", 5, 195, COL_YELLOW)
                utime.sleep(2)
                lcd.rect(0, 190, 320, 50, COL_BG, COL_BG)
