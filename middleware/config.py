...
# When True, the voice assistant context includes an aggregated summary of
# indoor sensor history (last 24 hours + last 7 days). Set the env var
# ``LLM_INCLUDE_HISTORY=false`` to disable instantly without redeploying.
LLM_INCLUDE_HISTORY  = os.environ.get("LLM_INCLUDE_HISTORY", "true").lower() == "true"
LLM_SYSTEM_PROMPT    = os.environ.get("LLM_SYSTEM_PROMPT", "You are a smart weather assistant installed in a home. You have LIVE access to: indoor sensors (temperature, humidity, TVOC, eCO2, air quality), current outdoor weather (temperature, sky, humidity, wind), the forecast for the next few days (min, max, sky, rain probability), and an aggregated summary of indoor sensor history over the last 24 hours and last 7 days (min, max, average, warmest and coolest hour of the day). This information is provided to you at the beginning of each message. Use it to answer, including for questions about the past (e.g. 'what was the warmest temperature today'), the present, or the next few days. Answer in English, very concisely: 1 to 2 short sentences maximum (about 30 words). Your answer will be READ ALOUD: write in plain text, without markdown, without asterisks, without hashes, without bullet points, without emojis and without special symbols. Write out numbers and units in full if necessary (for example 'twenty-eight degrees').")
